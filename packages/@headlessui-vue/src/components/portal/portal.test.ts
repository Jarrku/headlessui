...
it('should be possable to force the Portal into a specific element using PortalGroup', async () => {
  renderTemplate({
    template: html`
      <main>
  ...
  expect(document.body.innerHTML).toMatchInlineSnapshot(
    `"<div><div><div data-v-app=\\"\\"></div></div><main><aside id=\\"group-1\\"> A Next to A</aside><section id=\\"group-2\\"><span>B</span></section><!--teleport start--><!--teleport end--></main></div>"`
  )
})
